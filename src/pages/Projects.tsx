...
const Projects: React.FC = () => {
  ...
  const generatePDF = async (project: Project) => {
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          logoImg.onload = () => {
            ...
            // Desenhar a imagem com transparência personalizada
    ...
    doc.text(docTitle, 20, pdfSettings.header.height + 13);
    
    // Número do projeto
    doc.setFontSize(11);
    doc.text(`Nº ${project.number.toString().padStart(4, '0')}`, 160, pdfSettings.header.height + 13);
    
    // Informações do cliente (mais compacto)
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('DADOS DO CLIENTE', 20, pdfSettings.header.height + 25);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`Cliente: ${project.client_name}`, 20, pdfSettings.header.height + 33);
    doc.text(`Data: ${new Date().toLocaleDateString('pt-BR')}`, 140, pdfSettings.header.height + 33);
    
    // Descrição do projeto (mais compacto)
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('DESCRIÇÃO', 20, pdfSettings.header.height + 43);
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`${project.title}`, 20, pdfSettings.header.height + 51);
    
    // Quebra de linha para descrição longa
    const splitDescription = doc.splitTextToSize(project.description, 170);
    doc.text(splitDescription, 20, pdfSettings.header.height + 57);
    
    let yPosition = pdfSettings.header.height + 57 + (splitDescription.length * 4) + 10;
